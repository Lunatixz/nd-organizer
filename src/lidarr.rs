...
pub mod host_lidarr {
    ...
    pub fn find_album(cfg: &Config, album: &str, artist: &str) -> Option<LidarrAlbum> {
        ...
        let _ = host::kvstore::set_with_ttl(&cache_key, cached.to_string().into_bytes(), 86_400);
        found
    }
    ...
    pub fn force_search(cfg: &Config, album_id: i64) -> Result<(), String> {
        ...
        let body = format!("{{\"name\":\"AlbumSearch\",\"albumIds\":[{album_id}]}}");
        let req = host::http::HTTPRequest {
            method: "POST".into(),
...
mod tests {
    ...
    fn parses_album_resource() {
        let v = json!({
            ...
            "title": "The Wall",
            "monitored": true,
            "artist": { "artistName": "Pink Floyd", "monitored": true },
    ...
    fn parses_unmonitored_and_missing_stats() {
        let v = json!({
            ...
            "title": "EP",
            "monitored": false,
            "artist": { "artistName": "Someone", "monitored": true }
        });
        let a = parse_album(&v).unwrap();
        assert!(!a.monitored);
