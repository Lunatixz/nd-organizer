...
fn index_file(library_id: i32, rel: &str, abs: &Path) -> Result<(), String> {
    ...
    host::kvstore::set(&key, entry.to_string().into_bytes()).map_err(|e| e.to_string())
}
...
pub fn scan_step(cfg: &Config, library_id: i32) -> Result<bool, String> {
    ...
    if hit_limit {
        host::kvstore::set(&key, serde_json::to_vec(&stack).unwrap_or_default())
            .map_err(|e| e.to_string())?;
        ...
        let _ = host::kvstore::set(&format!("scan.donev2.{library_id}"), b"1".to_vec());
...
fn post_scan_status(cfg: &Config, library_id: i32, chunk: usize) {
    let total = {
        ...
        let _ = host::kvstore::set(&key, new.to_string().into_bytes());
        new
    };
...
pub fn identify_file(cfg: &Config, abs_path: &str) -> Option<(String, Option<String>)> {
    ...
    let _ = host::kvstore::set_with_ttl(&cache_key, cached.to_string().into_bytes(), 7 * 24 * 3600);
    result
}
...
pub fn group_step(cfg: &Config, library_id: i32) -> Result<usize, String> {
    ...
    let keys = host::kvstore::list(&prefix).map_err(|e| e.to_string())?;
