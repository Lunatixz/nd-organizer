...
pub mod host_stats {
    ...
    fn bump(key: &str, delta: i64) {
        let cur: i64 = host::kvstore::get(key)
            .ok()
            .flatten()
            .and_then(|v| String::from_utf8_lossy(&v).parse().ok())
            .unwrap_or(0);
        let _ = host::kvstore::set(key, (cur + delta).max(0).to_string().into_bytes());
    }
    ...
    fn observe(user: &str, threshold_pct: i32) -> Result<(usize, usize), String> {
        ...
        let previous: Vec<NowPlayingEntry> = host::kvstore::get(&key)
            .ok()
            .flatten()
        ...
        let _ = host::kvstore::set(&key, serde_json::to_vec(&current).unwrap_or_default());
        Ok((plays, skips))
    }

    fn all_weights() -> Vec<(String, f64, i64, i64)> {
        let mut weights = Vec::new();
        if let Ok(keys) = host::kvstore::list("stat.play.") {
            let vals = host::kvstore::get_many(keys).unwrap_or_default();
            for (k, v) in vals {
                let mfid = k.strip_prefix("stat.play.").unwrap_or(&k).to_string();
                let plays = String::from_utf8_lossy(&v).parse::<i64>().unwrap_or(0);
                let skips = host::kvstore::get(&skip_key(&mfid))
                    .ok()
                    .flatten()
                    .and_then(|v| String::from_utf8_lossy(&v).parse().ok())
                    .unwrap_or(0);
    ...
    pub fn refresh_top_picks(cfg: &Config, count: usize) -> Result<usize, String> {
        ...
        if let Some(id) = host::kvstore::get("stat.playlist.id").ok().flatten() {
        ...
        if let Some(pid) = parse_playlist_id(&resp) {
            let _ = host::kvstore::set("stat.playlist.id", pid.into_bytes());
        }
        Ok(top.len())
    }
