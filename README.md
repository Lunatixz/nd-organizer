...
| `ghcr.io/lunatixz/nd-organizer/filter:latest` | Subsonic filtering proxy — sits in front of Navidrome; drops filler tracks from the queue and skip-heavy tracks past the cap, re-sorts lists by weight, without touching files. |
...
- Plugin flagging: set `filterUrl = http://nd-organizer-filter:4534` and enable
...
- If **Lidarr** or **AudioMuse-AI** also run as containers, attach them to the
  same `media` network and set their URLs to container names:
...
- To attach an already-running container to the network without recreating it:
  ```bash
  docker network connect media <container-name>
  ```
...
| `filterUrl` | `""` | Subsonic filter proxy base URL (e.g. `http://nd-organizer-filter:4534`); receives `POST /filters`. |
